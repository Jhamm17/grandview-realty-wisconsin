...
async function setupTeamDatabase() {
  try {
    ...
    const agentsData = [
      {
        ...
        image_url: '/agents/christopher-lobrillo.png',
      ...
      },
      {
        ...
        image_url: '/agents/lynda-werner.png',
    ...
    if (agentsError) {
      console.error('Error checking agents table:', agentsError);
    } else {
      console.log(`✓ Agents table is accessible (${agents.length} agents found)`);
    }
    ...
    if (officeStaffError) {
      console.error('Error checking office_staff table:', officeStaffError);
    } else {
      console.log(`✓ Office staff table is accessible (${officeStaff.length} staff found)`);
    }
